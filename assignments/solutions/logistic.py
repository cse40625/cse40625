...
class LogisticRegression(object):
    ...
    def _compute_gradient(self, X, y):
        ...
        """
        y = y[:, np.newaxis]
        z = y * np.dot(X, self.weight_.T)[:, np.newaxis]
        grad = (y * X) * sigmoid(-z)
    ...
    def _update_weight(self, grad):
        ...
        grad : array, shape = [n_weights,]
            Gradient of the error.
        """
        self.weight_ += self.learning_rate * -grad
    ...
    def _decision_function(self, X):
        ...
        """
        return sigmoid(np.dot(X, self.weight_.T))
    ...
    def fit(self, X, y):
        ...
        # Insert bias term.
        bias = np.ones(shape=(n_instances, 1))
        X_1 = np.hstack((bias, X))

        # Initialize weight vector.
        self.weight_ = np.random.normal(size=n_features + 1)

        for i in range(self.max_iter):
            grad = self._compute_gradient(X_1, y)
            self._update_weight(grad)
    ...
    def predict(self, X):
        ...
        y_pred = np.sign(self._decision_function(X_1) - 0.5)
        return y_pred
